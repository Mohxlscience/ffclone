! function() {
    ...
    let c = {
        ...
        n = {
            ...
            toggleLocked: function() {
                "float" == m.type ? (m.direction = "from", c.to.amount.setAttribute("readonly", "readonly")) : m.lockType || c.to.amount.removeAttribute("readonly"), l.togClass(c.from.amount.parentNode, "locked", "from" == m.direction), l.togClass(c.to.amount.parentNode, "locked", "to" == m.direction)
            },
            toggleExtra: function() {
                var e = c.to.selector.val(),
                    e = (c.to.selector.options[e].network, c.to.selector.options[e].tag);
                l.togClass(c.tag.wrapper, "active", !!e), l.togClass(c.address.field, "with-extra", !!e), e ? (c.tag.label && (c.tag.label.innerHTML = e + " (optional)"), c.tag.field && c.tag.field.setAttribute("placeholder", e), clearTimeout(a.overflowAnimation), a.overflowAnimation = setTimeout(function() {
        ...
        s = {
            ...
            getInvoice: function() {
                ...
                "fixed" == m.type && (t.amount = ~~(1e8 * +m.to.amount)), a.keyupWallet = 1, c.address.field.blur(), s.makeInvoice(t).then(function(e) {
                    e = e.paymentRequest;
            ...
            confirmGenerate: function(e) {
                UI.popup({
                    html: '<div class="popup-content popup-confirm fix-width"><p>' + d.webln_popup_gen + '</p><div class="popup-ctrl"><button type="button" class="btn cancel">' + d.webln_popup_cancel + '</button><button type="button" class="btn submit popup-close-btn">' + d.webln_popup_gen_ok + "</button></div></div>",
                    afterRender: function() {
                        var e = this;
                        l.on(e.container, "click", ".cancel", function() {
                            s.allow = !1, setTimeout(function() {
                                s.allow = !0
                            }, 5e3), c.address.field && c.address.field.focus(), e.close()
            ...
            confirmSwitchFixed: function() {
                var e = {
                    ok: d.webln_popup_switch_ok,
                    cancel: d.webln_popup_cancel
                };
                UI.confirm(d.webln_popup_switch, function() {
    ...
    var i = function(e, t) {
            ...
            t.ctrlKey || t.altKey || t.metaKey || (48 != o && 96 != o || "0" != e.value || (e.value = "0."), 58 <= o && o <= 90 || 106 <= o && o <= 111 || 146 <= o ? (t.preventDefault(), 188 != o && 190 != o && 191 != o && 110 != o || -1 != e.value.search(/\./i) || (t = l.carPos(e), e.value = e.value.substr(0, t) + "." + e.value.substr(t++), l.setCarPos(e, t, t))) : 13 == o && e.blur())
        },
        u = function(t, o) {
            m.direction = m.lockSend || m.lockReceive ? m.direction : o, l.remClass(t.parentNode, "error"), clearTimeout(a.keyupAmount), n.toggleLocked(), t.value && (a.keyupAmount = setTimeout(function() {
                m[o].amount = t.value;
    ...
    let m = {
        ...
        checkType: function(e) {
            e ? (c.to.amount.value = e, m.type = "fixed", c.type.radioui && c.type.radioui.val("fixed"), m.direction = "to", n.toggleLocked(), c.type.float && (c.type.float.disabled = !0), c.to.amount.disabled = !0, c.from.amount.disabled = !0, m.address.fixedAmount = !0, m.getRatePaste()) : m.unlockAmounts()
        },
        pasteAddress: function(e, t) {
            c.address.field ? (c.address.field.value = e, t ? (c.address.field.focus(), n.resizeTextarea(e), c.address.field.blur()) : (l.remClass(c.address.field, "error"), m.validateAddress(), n.resizeTextarea(e))) : m.address.value = e
        },
        ...
        pasteAddressBook: function(t) {
            function e(e) {
                ...
                return l.addClass(t, "row-address"), t.setAttribute("data-addr", e.address), t.setAttribute("data-tag", e.tag), t.setAttribute("data-coin", e.coin), t.setAttribute("data-network", e.network), t.innerHTML = l.fmt(d.addressbook_row, e), t
            }
        ...
        closeAddressBook: function(e, t) {
            m.address.findfavorite = !1, c.address.addressbook.innerHTML = "", l.remClass(c.address.field.parentNode.parentNode, "show-address-book"), clearTimeout(a.closeAnimation), a.closeAnimation = setTimeout(function() {
                l.remClass(c.tag.wrapper, "noanim")
            }, 300), c.address.field.setAttribute("placeholder", l.fmt(d.address_ph, c.to.selector.selectedAttr("label"))), t && (c.tag.field.value = t), e ? m.pasteAddress(e) : m.pasteAddress("", !1)
        },
        checkAddress: function() {
            ...
            m.address.value && (e = {
                ...
                c.address.error && (c.address.error.innerHTML = ""), c.address.field && l.remClass(c.address.field, "error"), c.tag.field && l.remClass(c.tag.field, "error"), m.checkType(e.data.amount), m.error.address = !1, m.checkAddressInProgress = !1, n.validateSubmit()
            }).catch(function(e) {
        ...
        validateAddress: function() {
            !c.address.field || m.address.value == c.address.field.value && m.address.extra == c.tag.field.value || (c.address.field && (m.address.value = c.address.field.value), c.tag.field && (m.address.extra = c.tag.field.value), m.address.value ? -1 == a.keyupWallet && m.checkAddress() : (m.error.address = !0, m.unlockAmounts(), n.validateSubmit()))
        },
        validateExtra: function() {
            c.tag.field && (m.address.extra = c.tag.field.value);
            let e = m.address.extra;
            if (e) switch (m.to.ccy) {
                case "XRP":
                ...
                if (l.remClass(c.to.amount.parentNode, "error"), l.remClass(c.from.amount.parentNode, "error"), l.remClass(c.to.amount.parentNode, "offline"), l.remClass(c.from.amount.parentNode, "offline"), t) {
                    if (t.to.max = l.fixNumber(t.to.max, 8), m.error.from = !1, m.error.to = !1, t.errors.length) {
                        if (t.errors.includes("MAINTENANCE_FROM")) {
                            let e = "";
                            t.from.recoverytime && (r = moment.unix(t.from.recoverytime), o = moment().diff(r, "hours"), r = moment().diff(r, "days"), e = 1 < r ? "<br> " + l.fmt(d.recoverytime_days, r) : 1 == r ? "<br> " + l.fmt(d.recoverytime_day) : 1 < o ? "<br> " + l.fmt(d.recoverytime_hours, o) : "<br> " + l.fmt(d.recoverytime_hour)), c.from.error.innerHTML = d.maintenance + e, m.error.from = "OFFLINE"
                        } else t.errors.includes("OFFLINE_FROM") ? (c.from.error.innerHTML = d.ccy_offline, m.error.from = "OFFLINE") : t.errors.includes("RESERVE_FROM") && (c.from.error.innerHTML = d.ccy_reserve, m.error.from = "OFFLINE");
                        if (t.errors.includes("MAINTENANCE_TO")) {
                            let e = "";
                            t.to.recoverytime && (r = moment.unix(t.to.recoverytime), o = moment().diff(r, "hours"), r = moment().diff(r, "days"), e = 1 < r ? "<br> " + l.fmt(d.recoverytime_days, r) : 1 == r ? "<br> " + l.fmt(d.recoverytime_day) : 1 < o ? "<br> " + l.fmt(d.recoverytime_hours, o) : "<br> " + l.fmt(d.recoverytime_hour)), c.to.error.innerHTML = d.maintenance + e, m.error.to = "OFFLINE"
                        } else t.errors.includes("OFFLINE_TO") ? (c.to.error.innerHTML = d.ccy_offline, m.error.to = "OFFLINE") : t.errors.includes("RESERVE_TO") && (c.to.error.innerHTML = d.ccy_reserve, m.error.to = "OFFLINE");
                        "OFFLINE" != m.error.from && "OFFLINE" != m.error.to && (t.errors.includes("LIMIT_MIN") ? (c[s].error.innerHTML = l.fmt(d.limit_min, '<button type="button" class="maxmin-value" data-value="' + t[s].min + '">' + t[s].min + " " + t[s].coin + "</button>"), m.error[s] = !0) : t.errors.includes("LIMIT_MAX") && (c[s].error.innerHTML = l.fmt(d.limit_max, '<button type="button" class="maxmin-value" data-value="' + t[s].max + '">' + t[s].max + " " + t[s].coin + "</button>"), m.error[s] = !0)), m.error.from && (l.addClass(c.from.amount.parentNode, "error"), l.togClass(c.from.amount.parentNode, "offline", "OFFLINE" == m.error.from)), m.error.to && (l.addClass(c.to.amount.parentNode, "error"), l.togClass(c.to.amount.parentNode, "offline", "OFFLINE" == m.error.to))
                    }
                    n.validateSubmit(), i ? (c.from.amount.value = t.from.amount, c.to.amount.value = e + t.to.amount) : "to" == s ? (c.from.amount.value = t.from.amount, c.to.amount.value = e + l.fixNumber(t.to.amount)) : (document.activeElement != c.from.amount && (c.from.amount.value = l.fixNumber(t.from.amount)), c.to.amount.value = e + t.to.amount), m.from.amount = l.fixNumber(t.from.amount), m.to.amount = l.fixNumber(t.to.amount), m.usd.from = t.from.usd, m.usd.to = t.to.usd, c.from.rate.innerHTML = "1 " + t.from.coin + " " + a + " " + t.from.rate + " " + t.to.coin, c.from.maxmin.innerHTML = l.fmt(d.minmax, t.from), c.to.rate.innerHTML = "1 " + t.to.coin + " " + a + " " + t.to.rate + " " + t.from.coin, c.to.maxmin.innerHTML = l.fmt(d.minmax, t.to), c.rate_usd_from.innerHTML = "$â€¯" + l.round(t.from.usd, 2), c.rate_usd_to.innerHTML = "$â€¯" + l.round(t.to.usd, 2), n.toggleLocked(), t.ccies && m.avaliblesCurrencies(t.ccies)
        ...
        init: function(t) {
            if (c.container = l.id("exchange_amount"), c.agreeterms = l.id("agreeterms"), c.type.radioui = UI.radio("select_type_from"), c.btn_exchange = l.id("exchange_submit"), c.btn_reverse = l.id("btn_reverse"), c.rate_usd_from = l.id("rate_usd_from"), c.rate_usd_to = l.id("rate_usd_to"), c.from.amount = l.id("select_amount_from"), c.from.name = l.id("select_ccyname_from"), c.from.label = l.id("select_label_from"), c.from.reverse = l.id("btn_reverse_from"), c.from.logo_t = l.id("logo_text_from"), c.from.logo_a = l.id("logo_arrow_from"), c.from.error = l.id("select_hinterror_from"), c.from.rate = l.id("select_rate_from"), c.from.maxmin = l.id("select_maxmin_from"), c.from.info = l.id("select_info_from"), c.to.amount = l.id("select_amount_to"), c.to.name = l.id("select_ccyname_to"), c.to.label = l.id("select_label_to"), c.to.reverse = l.id("btn_reverse_to"), c.to.logo_t = l.id("logo_text_to"), c.to.logo_a = l.id("logo_arrow_to"), c.to.error = l.id("select_hinterror_to"), c.to.rate = l.id("select_rate_to"), c.to.maxmin = l.id("select_maxmin_to"), c.to.info = l.id("select_info_to"), c.address.field = l.id("receive_wallet"), c.address.label = l.id("receive_wallet_label"), c.address.error = l.id("receive_wallet_error"), c.address.hidden = l.id("receive_wallet_hidden"), c.address.func.paste = l.id("wallet_paste"), c.address.func.scanqr = l.id("wallet_scanqr"), c.address.func.favorite = l.id("wallet_favorite"), c.address.func.clean = l.id("wallet_clear"), c.address.addressbook = l.id("wallet_addressbook"), c.tag.field = l.id("receive_extraid"), c.tag.label = l.id("receive_extraid_label"), c.tag.wrapper = l.id("wallet_extra_outer"), c.tag.error = l.id("receive_extraid_error"), c.tag.func.paste = l.id("extraid_paste"), c.tag.func.clean = l.id("extraid_clear"), c.type.fixed = l.id("fixed_type"), c.type.float = l.id("float_type"), m.from.req = t.fromAmount || m.from.req, m.to.req = t.toAmount || m.to.req, m.lockAmount = (void 0 !== t.lockAmount ? t : m).lockAmount, m.lockType = (void 0 !== t.lockType ? t : m).lockType, m.lockSend = (void 0 !== t.lockSend ? t : m).lockSend, m.lockReceive = (void 0 !== t.lockReceive ? t : m).lockReceive, m.lockAddress = (void 0 !== t.lockAddress ? t : m).lockAddress, m.errorIgnore = (void 0 !== t.errorIgnore ? t : m).errorIgnore, m.floatLNInvoice = (void 0 !== t.floatLNInvoice ? t : m).floatLNInvoice, d.selector_option = t.selector_tmpl || d.selector_option, d.selector_label = t.label_tmpl || d.selector_label, d.search_ph = t.search_ph || d.search_ph, d.search_found = t.search_found || d.search_found, d.address_ph = t.address_ph || d.address_ph, d.invoice_ph = t.invoice_ph || d.invoice_ph, d.favaddress_ph = t.favaddress_ph || d.favaddress_ph, d.address_invalid = t.address_invalid || d.address_invalid, d.route_invalid = t.route_invalid || d.route_invalid, d.address_error = t.address_error || d.address_error, d.invoice_error = t.invoice_error || d.invoice_error, d.limit_min = t.limit_min || d.limit_min, d.limit_max = t.limit_max || d.limit_max, d.addressbook_row = t.address_tmpl || d.address_tmpl, d.addressbook_fav = t.addressbook_fav || d.addressbook_fav, d.addressbook_last = t.addressbook_last || d.addressbook_last, d.maintenance = t.maintenance || d.maintenance, d.ccy_offline = t.ccy_offline || d.ccy_offline, d.ccy_reserve = t.ccy_reserve || d.ccy_reserve, d.qrcode_error = t.qrcode_error || d.qrcode_error, d.recoverytime = t.recoverytime || d.recoverytime, d.recoverytime_hour = t.recoverytime_hour || d.recoverytime_hour, d.recoverytime_hours = t.recoverytime_hours || d.recoverytime_hours, d.recoverytime_day = t.recoverytime_day || d.recoverytime_day, d.recoverytime_days = t.recoverytime_days || d.recoverytime_days, d.clipboard_permission = t.clipboard_permission || d.clipboard_permission, d.popup_warning_fee = t.popup_warning_fee || d.popup_warning_fee, d.popup_forbidden_usa = t.popup_forbidden_usa || d.popup_forbidden_usa, c.type.radioui ? m.type = c.type.radioui.val() : t.type && (m.type = t.type), c.from.selector = UI.selector("select_currency_from", {
                    ...
                    var t;
                    (e = e || event).preventDefault(), l.hasClass(this, "disabled") || (t = c.from.selector.val(), e = c.to.selector.val(), c.to.selector.options[t] && c.from.selector.options[e] && (c.to.selector.val(t), m.pasteAddress("", !1)))
                    ...
                    if (WebLN && s.allow && "btcln" == m.to.ccy && "" == c.address.field.value) try {
                    ...
                    var o = this.value;
                    n.resizeTextarea(o), m.address.findfavorite ? a.keyupWallet = setTimeout(function() {
                        a.keyupWallet = -1;
                    ...
                    }, 1e3) : o ? a.keyupWallet = setTimeout(function() {
                        a.keyupWallet = -1, m.validateAddress()
                    }, 1e3) : (m.error.address = !0, m.unlockAmounts(), n.validateSubmit(), m.checkAddressInProgress = !1)
                }), l.bind(c.address.field, "blur", function() {
                    m.address.findfavorite || m.validateAddress()
                }), l.bind(c.tag.field, "input", function() {
                    l.remClass(this, "error"), l.remClass(c.address.field, "error"), c.address.field.value && (m.checkAddressInProgress = !0, clearTimeout(a.keyupWallet), a.keyupWallet = setTimeout(function() {
                        a.keyupWallet = -1, m.validateAddress()
                    }, 1e3))
                    ...
                    m.address.findfavorite = !0, l.addClass(c.address.field.parentNode.parentNode, "show-address-book"), l.addClass(c.tag.wrapper, "noanim"), l.addClass(c.address.addressbook, "loading"), l.remClass(c.address.field, "error"), c.address.field.setAttribute("placeholder", l.fmt(d.favaddress_ph, c.to.selector.selectedAttr("label")));
                ...
                }), navigator.clipboard && (l.remClass(c.address.func.paste, "none"), l.remClass(c.tag.func.paste, "none"), l.bind(c.address.func.paste, "click", function(e) {
                    a.stop("keyupWallet"), navigator.clipboard.readText().then(function(e) {
                        m.pasteAddress(e)
                    }).catch(function(e) {
                    ...
                    (e = e || event).preventDefault(), UI.qrscan({
                        scan: function(e) {
                            e = e.match(/(?:\w+:)?(\w+)(?:\?.*)?/i);
                    ...
                    clearTimeout(a.keyupWallet), a.keyupWallet = setTimeout(function() {
                        a.keyupWallet = -1
                    }, 100)
                    ...
                    if (0 == (r = r || event).button) {
                        ...
                        if (!m.lockAmount) return "float" == m.type && "to" == t || (e.value = o), n.toggleLocked(), a.setAmount = setTimeout(function() {
                            a.setAmount = -1
                        }, 300), m.getRate(o, t), !1
                    }
                    ...
                    for (var o in e.preventDefault(), m.error)
                        if (m.error[o]) {
                            ...
                            switch (o) {
                                ...
                                case "address":
                                default:
                                    -1 != a.keyupWallet || m.checkAddressInProgress ? e = !1 : (c.address.field ? (c.address.error && (c.address.error.innerHTML = l.fmt(d.address_error, c.to.selector.selectedAttr("label"))), c.address.field.focus()) : UI.alert(d.address_invalid), l.addClass(c.address.field, "error"))
                            }
                            if (e && !m.errorIgnore) return
                        } if (!c.agreeterms || c.agreeterms.checked) {
                        let e = {
                            fromCcy: m.from.ccy.toUpperCase(),
                            toCcy: m.to.ccy.toUpperCase(),
                            type: m.type,
                            toAddress: m.address.value,
                            platform: window.navigator.platform,
                            tz: Intl.DateTimeFormat().resolvedOptions().timeZone,
                            ts: ~~(Date.now() / 1e3),
                            tt: Date()
                        };
                        "to" == m.direction ? e.toQty = m.to.amount : e.fromQty = m.from.amount, c.to.selector.options[e.toCcy].tag && (e.tag = c.tag.field.value), "function" == typeof gtag && gtag("event", "exchange", {
                            event_category: "button"
                        });
                        m.usd.to, m.usd.from;
                        .07 <= 1 - m.usd.to / m.usd.from && 2 < m.usd.from - m.usd.to && d.popup_warning_fee ? UI.popup({
                            html: d.popup_warning_fee,
                            class: "foreground",
                            onSubmit: function() {
                                t.loading(), m.createOrder(e, t)
                            }
                        }).show() : (t.loading(), m.createOrder(e, t))
            ...
            m.isInit = !0, t.address && (m.error.address = !1), m.pasteAddress(t.address || "", !1), m.address.extra = t.extra || "", c.tag.field && (c.tag.field.value = m.address.extra), m.lockReceive && m.lockAmount && (m.direction = "to", m.checkType(t.toAmount));
    ...
    l.bind(window, "resize", function() {
        n.resizeTextarea(c.address.field.value)
    }), window.Exchange = m, window.ExObj = c
}();